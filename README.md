...
`app/pages/api-reference/[...slug].vue` is that page. It queries the generated Markdown as a Nuxt Content AST and rewrites it before rendering: `app/utils/mdc.ts` holds the AST primitives, `app/utils/api-document.ts` the schema-aware queries and transforms, and `useApiNavigation` derives the sidebar tree from the content paths. The GraphQL definition and the examples are lifted out of the prose into `SchemaCodeCard`s, highlighted with [Shiki](https://shiki.style/); on operation pages the request and its response are shown as a [code group](https://ui.nuxt.com/docs/typography/code-group).
...
| `npm run generate` | Builds the static site to `./.output/public`                            |
